...
for j in names:
    ...
    if (station == '420'):
        p1 = pd.read_csv(dir2 + '\\opt_ris_par' + station_c, index_col=0)
        p2 = pd.read_csv(dir2 + '\\opt_rec_par' + station_c, index_col=0)
    elif (station == '3421'):
        p1 = pd.read_csv(dir2 + '\\opt_ris_par' + station_c, index_col=0)
        p2 = pd.read_csv(dir2 + '\\opt_rec_par' + station_c, index_col=0)
    elif (station == '3465'):
        p1 = pd.read_csv(dir1 + '\\opt_ris_par' + station_c, index_col=0)
        p2 = pd.read_csv(dir1 + '\\opt_rec_par' + station_c, index_col=0)
    elif (station == '3470'):
        p1 = pd.read_csv(dir1 + '\\opt_ris_par' + station_c, index_col=0)
        p2 = pd.read_csv(dir1 + '\\opt_rec_par' + station_c, index_col=0)

    tt_R = p1.at['tt', 'val']
    cmelt_R = p1.at['cmelt', 'val']
    FC_R = p1.at['FC', 'val']
    Beta_R = p1.at['Beta', 'val']
    PWP_R = p1.at['PWP', 'val']
    L1_R = p1.at['L1', 'val']
    K_uu_R = p1.at['K_uu', 'val']
    K_ul_R = p1.at['K_ul', 'val']
    K_d_R = p1.at['K_d', 'val']
    k_ll_R = p1.at['k_ll', 'val']

    tt_r = p2.at['tt', 'val']
    cmelt_r = p2.at['cmelt', 'val']
    FC_r = p2.at['FC', 'val']
    Beta_r = p2.at['Beta', 'val']
    PWP_r = p2.at['PWP', 'val']
    L1_r = p2.at['L1', 'val']
    K_uu_r = p2.at['K_uu', 'val']
    K_ul_r = p2.at['K_ul', 'val']
    K_d_r = p2.at['K_d', 'val']
    k_ll_r = p2.at['k_ll', 'val']

    tt_t = np.mean(np.array([tt_R, tt_r]))
    cmelt_t = np.mean(np.array([cmelt_R, cmelt_r]))
    FC_t = np.mean(np.array([FC_R, FC_r]))
    Beta_t = np.mean(np.array([Beta_R, Beta_r]))
    PWP_t = np.mean(np.array([PWP_R, PWP_r]))
    L1_t = np.mean(np.array([L1_R, L1_r]))
    K_uu_t = np.mean(np.array([K_uu_R, K_uu_r]))
    K_ul_t = np.mean(np.array([K_ul_R, K_ul_r]))
    K_d_t = np.mean(np.array([K_d_R, K_d_r]))
    k_ll_t = np.mean(np.array([k_ll_R, k_ll_r]))
