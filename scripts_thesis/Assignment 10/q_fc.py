...
ppt_diff = np.diff(rs[:, prec_col])

Ris_val_per = 30
rec_val_per = 30

pos = np.where(ppt_diff > 0, 1, 0)
neg = np.where(ppt_diff < 0, 1, 0)
pos_elm = np.multiply(pos, ppt_diff)
neg_elm = np.multiply(neg, ppt_diff)
pos_arr = np.delete(pos_elm, np.where(pos_elm == 0), axis=0)
neg_arr = np.delete(neg_elm, np.where(neg_elm == 0), axis=0)
ppt_diff_contains = {'zeros':np.count_nonzero(ppt_diff == 0),
'+ve_#' :pos_arr.size, '-ve_#':neg_arr.size}
print('ppt_diff_contains = ', ppt_diff_contains)
print('\n')
ppt_sort_asc = np.sort(pos_arr)
ppt_sort_des = -np.sort(-neg_arr)
R_indc = int((ppt_sort_asc.size - (Ris_val_per / 100) * ppt_sort_asc.size))
r_indc = int((ppt_sort_des.size - (rec_val_per / 100) * ppt_sort_des.size))
rising_limit = ppt_sort_asc[R_indc]
recession_limit = ppt_sort_des[r_indc]
...
# flag
if rising_flag:
    rissing_Beta = np.where(ppt_diff >= rising_limit, 1, 0)
else:
    rissing_Beta = np.where(ppt_diff >= rising_limit, 0, 0)
if recession_flag:
    recession_Beta = np.where(ppt_diff <= recession_limit, 1, 0)
else:
    recession_Beta = np.where(ppt_diff <= recession_limit, 0, 0)
if threshhold_flag:
    threshhold_Beta = (np.where((ppt_diff > recession_limit) &
        (ppt_diff < rising_limit), 1, 0))
else:
    threshhold_Beta = (np.where((ppt_diff > recession_limit) &
        (ppt_diff < rising_limit), 0, 0))
...
for i in range(strt_idx, rs.shape[0]):
    ...
    if cr[prec_col] - pr[prec_col] >= rising_limit:
        Beta = Beta_1
    elif cr[prec_col] - pr[prec_col] <= recession_limit:
        Beta = Beta_2
    else:
        Beta = Beta_3
