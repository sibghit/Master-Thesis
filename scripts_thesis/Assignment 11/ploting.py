...
r'UOS Germany/Thesis/Assignment 11/'))
...
ps_df['obs_sim_sq'] = (ps_df['qob_col'] - ps_df['q_simq_col']) ** 2
obs_sim_sq_total = ps_df['obs_sim_sq'].sum()
ps_df['%obs_sim_sq'] = (ps_df['obs_sim_sq'] / obs_sim_sq_total) * 100
print(ps_df.sort_values(by='qob_col'))
# ps_df.to_excel('obs_sim_sq.xlsx', sheet_name='Sheet1')

for i in range(int(ps_df.index[1].strftime('%Y')),
    int(ps_df.index[-1].strftime('%Y')) + 1, 2):
